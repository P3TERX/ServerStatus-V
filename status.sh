...
Set_password(){
	...
	stty erase '^H' && read -p "(默认: p3terx.com):" password_s
	[[ -z "$password_s" ]] && password_s="p3terx.com"
	echo && echo "	================================================"
...
Install_caddy(){
	...
	if [[ "${caddy_yn}" == [Yy] ]]; then
		...
		if [[ ! -e "/usr/local/caddy/caddy" ]]; then
			wget -N --no-check-certificate https://raw.githubusercontent.com/P3TERX/doubi_backup/master/caddy_install.sh
